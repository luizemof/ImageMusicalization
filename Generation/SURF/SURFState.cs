...
namespace Generation.SURF
{
    ...
    public class SURFState : State<SURFResult>, IState
    {
        ...
        private double _CalculateProbability(SURFState linkedState)
        {
            ...
            _Log.WriteLog(string.Format("Próximo estado: {0}", linkedState.Element.Note), 1);
            _Log.WriteLog(string.Format("Número de elementos: {0}", numberOfElements), 2);
            _Log.WriteLog(string.Format("distance: {0}", distance), 2);
            _Log.WriteLog(string.Format("Probabilidade: {0}", roundValue), 2);
            return roundValue;
        }
    }
}
